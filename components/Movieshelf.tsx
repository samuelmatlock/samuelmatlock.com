          ...
          <HStack alignItems="center" gap={1} overflowX="hidden" cursor="grab" ref={viewportRef}>
            <div ref={trackRef} style={{ display: "flex", gap: "4px", flexShrink: 0, willChange: "transform" }}>
              {movies.map((movie, index) => (
                ...
                >
                  <Flex alignItems="flex-start" justifyContent="center" width={spineWidth} height={movieHeight} flexShrink={0} transformOrigin="right" backgroundColor={movie.spineColor} color={movie.textColor} transform={`rotateY(${movieIndex === index ? "-60deg" : "0deg"})`} transition="all 600ms cubic-bezier(0.4, 0, 0.2, 1)" willChange="auto" filter="brightness(0.8) contrast(2)" style={{ transformStyle: "preserve-3d" }}>
                  ...
                  <Box position="relative" flexShrink={0} overflow="hidden" transformOrigin="left" transform={`rotateY(${movieIndex === index ? "30deg" : "88.8deg"})`} transition="all 600ms cubic-bezier(0.4, 0, 0.2, 1)" willChange="auto" filter="brightness(0.8) contrast(2)" style={{ transformStyle: "preserve-3d" }}>
